...
describe User do
  ...
  # Check if required fields exist in table
  ...
  describe "when name is not present" do  # To test the validation in the model, we first set the name to be an empty string
    ...
    it { should_not be_valid }            #  If the result is 'not valid' - then the test passes
  end

  # Test validation logic on email address
  ...
  describe "when name is too long" do
    before { @user.name = "a" * 51 }     # Set the length to 51 so the validation fails	
    it { should_not be_valid }
  end
